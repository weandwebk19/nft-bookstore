...
contract ListedBookStorage {
  ...
  function _createListedBook(uint tokenId, address seller, uint256 price, uint amount) private {
    ...
    _idToListedBook[_listedBooks.current()] = ListedBook(tokenId, seller, price, amount);
  ...
  function getIdListedBook(uint tokenId, address seller) public view returns(uint) {
    require(tokenId != 0 && seller != address(0), "Token id and address of you is invalid");
    return _allListedBook[tokenId][seller];
  }


  function getAmountOfListedBooks(uint256 tokenId, address owner) public view returns(uint) {
  ...
  function sellListedBooks(uint256 tokenId, uint price, uint256 amount, address seller) public payable {
    uint idListedBook = getIdListedBook(tokenId, seller);
    if (idListedBook == 0) {
  ...
  function updateListedBookFromSale(uint256 tokenId, uint newPrice, uint256 newAmount, address seller) public {
    uint idListedBook = getIdListedBook(tokenId, seller);
    require(idListedBook != 0,
  ...
  function buyListedBooks(uint256 tokenId, address seller, uint256 amount, address buyer, uint256 value) public payable returns(uint) {

    uint idListedBook = getIdListedBook(tokenId, seller);
    if (idListedBook != 0) {
      ListedBook memory listedBook = _idToListedBook[idListedBook];
      require(buyer != listedBook.seller, "You can't buy your own books on sale");
      require(value == amount * listedBook.price, "Please submit the asking price");
      require(amount <= listedBook.amount && amount > 0, "Amount is invalid");

      uint256 totalPrice = amount * listedBook.price;

      updateListedBookFromSale(tokenId, 
                              listedBook.price,
